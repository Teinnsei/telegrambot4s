...
package io.halahutskyi.telegrambot4s.core
...
import io.halahutskyi.telegrambot4s.message.event.document._
import io.halahutskyi.telegrambot4s.message.{TelegramCommand, TelegramEvent}
...
class TelegramActor(telegramEventBus: TelegramEventBus) extends Actor with ActorLogging {
  ...
  private val commandToUrl =
    new Reflections("io.halahutskyi.telegrambot4s.api")
